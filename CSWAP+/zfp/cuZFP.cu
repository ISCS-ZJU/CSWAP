...
cuda_compress(zfp_stream *stream, const zfp_field *field)
{
  ...
  if(field->type == zfp_type_float)
  {
    ...
    double cur_time = (end_.tv_sec*1000 + end_.tv_usec / 1000.0) - (start_.tv_sec*1000 + start_.tv_usec / 1000.0);
    printf("Float compress use time: %lf \n",cur_time);
  }
...
cuda_decompress(zfp_stream *stream, zfp_field *field)
{
  ...
  if(field->type == zfp_type_float)
  {
    ...
    double cur_time = (end_.tv_sec*1000 + end_.tv_usec / 1000.0) - (start_.tv_sec*1000 + start_.tv_usec / 1000.0);
    printf("Float decompress use time: %lf \n",cur_time);
    d_data = (void*) data;
  }
  else if(field->type == zfp_type_double)
  {
